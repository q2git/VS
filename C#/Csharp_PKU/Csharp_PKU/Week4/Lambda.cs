...
namespace Csharp_PKU.Week4
{
    public partial class Lambda : Form
    {
...

        {
            ...
            //匿名方法
            new Thread(new ThreadStart(delegate () { Console.Write("my function1"); })).Start();

            //匿名方法，省略参数列表

            new Thread(new ThreadStart(delegate { Console.Write("my function2"); })).Start();

            //匿名方法，自动转委托

            new Thread(delegate () { Console.Write("my function3"); }).Start();
            ...
            new Thread(
                () =>this.listBox1.Invoke(
                    new Action(
                        ()=>this.listBox1.Items.Add("my function4") //用多线程操作UI
                        )
                    )
                ).Start();
            //new Thread(() => { Console.Write("my function4"); }).Start();
...

        {
            ...
            foreach (string s in result2) listBox1.Items.Add(s);
            ...
            foreach (Book b in result3) listBox1.Items.Add(b.price + " ");
